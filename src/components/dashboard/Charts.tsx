 ...
 * Três visualizações: linha (vendas), barras (produtos) e área (horário)
 * Todos otimizados para mobile e usando verde oliva primário
 */
...
// ============================================
// Configuração comum de estilos para tooltips
// ============================================
const getTooltipStyle = (isMobile: boolean) => ({
  contentStyle: {
...
export function SalesChart({ data }: { data: SalesData[] }) {
  ...
  const tooltipStyle = getTooltipStyle(isMobile);
    ...
    <div className="kpi-card">
      ...
      <div className="h-[220px] sm:h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart 
            data={formattedData} 
            margin={{ top: 5, right: isMobile ? 5 : 20, left: isMobile ? -15 : 10, bottom: 5 }}
          >
            ...
            <YAxis 
              ...
              axisLine={false}
              tickLine={false}
              tickFormatter={(v) => isMobile ? `${(v/1000).toFixed(0)}k` : `R$${v}`}
              width={isMobile ? 35 : 60}
            />
            <Tooltip {...tooltipStyle} formatter={(v: number) => [`R$ ${v.toFixed(2)}`, 'Total']} />
            <Line
              type="monotone"
              dataKey="total"
          ...
          >
            ...
            <Bar dataKey="sales" radius={[0, 6, 6, 0]} maxBarSize={isMobile ? 24 : 32}>
              {formattedData.map((_, i) => (
                <Cell key={i} fill={`hsl(var(--primary) / ${1 - i * 0.2})`} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
    ...
    <div className="kpi-card">
      ...
      <div className="h-[220px] sm:h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart 
            data={data} 
            margin={{ top: 5, right: isMobile ? 5 : 20, left: isMobile ? -20 : 10, bottom: 5 }}
          >
            <defs>
